...
export function hasInspectedPage(inspection: TextInspection | null, page: number | null) {
  return !!inspection && page !== null && (!inspection.pages || inspection.pages.includes(page));
}
...
export function usePreparedText(
  ...
  useEffect(() => {
    if (!bytes || !pageCount || page === null || bytes.length > 10 * 1024 * 1024 || pageCount > 100)
      return;
    if (cache.current?.bytes !== bytes) cache.current = { bytes, inspection: restored };
    const entry = cache.current;
    if (
      restored &&
      (!entry.inspection ||
        !restored.pages ||
        entry.inspection.pages?.every((p) => restored.pages!.includes(p)))
    )
      entry.inspection = restored;
    ...
    if (hasInspectedPage(restored, page)) {
      entry.inspection = restored;
      setResult({ bytes, inspection: restored! });
      return;
    }
    const controller = new AbortController();
    setFailure(null);
    void requestTextPdf(bytes, name, { operation: 'inspect', page }, false, controller.signal)
  ...
  }, [bytes, name, pageCount, page, restored, retry]);
  ...
  const inspection =
    restored &&
    (!restored.pages || !prepared || prepared.pages?.every((p) => restored.pages!.includes(p)))
      ? restored
      : prepared;
  return {
    inspection,
