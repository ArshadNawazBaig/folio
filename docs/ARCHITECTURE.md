...
The dashboard offers paginated user/subscription directories, suspension, time-limited courtesy grants, cancellation/resumption, versioned pricing, site settings, and support conversations. Changes are audited. Stripe mutations use payload-bound UUID requests and Stripe idempotency keys; database publication uses an expected-version check to reject stale edits. Cross-provider Stripe/database operations are not a distributed transaction; failed publications can leave unused Stripe products/prices. Retry the same request after a transient failure and inspect the audit log.
