...
For PDF-to-Word, Excel, and PowerPoint, set `CLOUDCONVERT_API_KEY` with `task.read` and `task.write` scopes. Folio creates a conversion job, verifies and downloads the output, and attempts to delete the provider job afterwards. The existing `CONVERTAPI_TOKEN` remains an optional alternative when CloudConvert is not configured. See [PROCESSING-SERVICES.md](PROCESSING-SERVICES.md) for account setup, the recommended stack, validation and deployment limits. Both adapters currently accept at most 10 MB and 100 source pages; the Vercel transport limit is lower, as described below.
...
Deploy these routes on a Node server supporting child processes, the bundled PDFium WASM engine, request bodies up to 30 MB, and processing requests up to 180 seconds. Vercel's lower fixed body limits require direct private uploads, durable background jobs and signed downloads before these routes can offer their full application limits. Google, CloudConvert and ConvertAPI calls time out; failed or cancelled attempts may still incur provider fees. Provider retention terms apply separately from Folio's memory-only processing. Do not enable broad paid remote processing until this transport work and real-provider quality tests are complete.
